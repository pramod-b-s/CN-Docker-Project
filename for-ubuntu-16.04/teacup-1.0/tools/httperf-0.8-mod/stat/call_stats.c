...
typedef struct {
	Time c_req_time; /* time request was sent */
...
struct print_stats print_info[]={
	{"req time", 		"", 				"%-20s", 	"%-20f",		"NA"},
	{"call number", 		"", 				"%-13s", 	"%-13d",		"NA"},
	{"conn number", 		"", 				"%-13s", 	"%-13d", 		"NA"},
	{"request size", 		"(bytes)", 	"%-14s", 	"%-14d",		"NA"},
	{"response size", 	"(bytes)", 	"%-15s", 	"%-15d",		"NA"},
	{"% bytes recvd", 	"", 				"%-15s", 	"%-15.2f",	"NA"},
	{"byte rate", 			"(Mbps)", 	"%-11s", 	"%-11.1f",	"NA"},
	{"conn time", 			"(s)", 			"%-11s", 	"%-11.7f",	"NA"},
	{"response time", 	"(s)", 			"%-15s", 	"%-15.7f",	"NA"},
	{"total time", 			"(s)", 			"%-12s", 	"%-12.7f",	"NA"},
	{"HTTP status", 		"", 				"%-13s", 	"%-13d",		"NA"},
	{"time limit", 			"(s)", 			"%-11s", 	"%-11.7f",	"NA"},
	{"timed out", 			"", 				"%-11s", 	"%-11s",		"NA"},
	{"file requested", 	"", 				"%s", 		"%s",				"NA"},
	{0,0,0,0,0}
};
...
print_call (call_data cd)
{
	...
	if (cd.c_req_time != -1)
        {
                printf(print_info[i++].data_frmt, cd.c_req_time);
        }
        else
        {
                printf(print_info[i++].hdr_frmt, print_info[(i-1)].data_blank);
        }
	if (cd.c_call_id != -1)
	{
...
void track_call_request(Call *c )
{
	...
	if (index != -1)
	{
		...
		call_stats[index].c_req_time = c->basic.time_send_start;
...
void process_call_timeout(Conn * c )
{
	...
	{
		/* this is used if a wsesslog file was not specified */
	...
	* then the first call was already created when using wsesslog*/
	if (c->state < S_CONNECTED && param.wsesslog.num_sessions)
	{
