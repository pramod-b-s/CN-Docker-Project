...
#ifdef UW_STABLE_STATS
static int stable_conns_start;	/* number of connections to ignore at beginning */
static int stable_conns_end;	/* ignore connections starting from this id	*/
#endif /* UW_STABLE_STATS */
...
#ifdef UW_STABLE_STATS
  if (s->id >= stable_conns_start && s->id < stable_conns_end) {
    basic.is_stable = 1;
    s->basic.is_stable = 1;
...
stable_stats_init(void)
{
  ...
  printf( "Stable period is %d to %d connections\n", stable_conns_start, stable_conns_end );
}
#endif /* UW_STABLE_STATS */
...
// }
  ...
  printf("Overall reply rate: %.1f replies/sec\n",reply_rate_overall);
