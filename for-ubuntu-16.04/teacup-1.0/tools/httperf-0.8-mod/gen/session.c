...
create_conn (Sess *sess, struct Conn_Info *ci)
{
  ...
  ci->conn->basic.num_calls = sess->num_requests;
  ...
  ci->num_sent = 0;		/* (re-)send all pending calls */
	...
	if (sess->port != 0)
	{
		ci->conn->port = sess->port;
	}
	if (sess->server != NULL)
	{
		ci->conn->hostname = sess->server;
		ci->conn->hostname_len = strlen(sess->server);
	}
  
#ifdef HAVE_SSL
	/* Add SSL set-up for wsesslog file specified SSL sessions */
  if (sess->use_ssl == 1 && !ci->conn->ssl)
  {
    ci->conn->ssl = SSL_new (ssl_ctx);
    if (!ci->conn->ssl)
		{
    ...
    if (param.ssl_cipher_list)
		{
			...
			int ssl_err = SSL_set_cipher_list (ci->conn->ssl, param.ssl_cipher_list);
	...
	/* Remove SSL set-up for wsesslog file specified SSL sessions */
	else if (sess->use_ssl == 2 && ci->conn->ssl)
	{
		SSL_free(ci->conn->ssl);
		ci->conn->ssl = NULL;
	}
...
Call *
session_get_last_call (Conn *conn)
{
	assert (object_is_conn(conn));
	...
	ci = CONN_PRIVATE_DATA(conn)->ci;

	assert (ci != NULL);

	index = (ci->wr-1) % MAX_PIPED;
